...
# Only keep structures that contain a specific base pair or bulged nucleotide?
...
class TraceFile:
    ...
    def filter_specific_bp(self):
        for current_structure in self.raw_list:
            ...
            for entry in filter_bp:
                # if entry[0] not in current_structure[3] or current_structure[3][entry[0] - 1] != entry[1]:
                if current_structure[3][entry[0] - 1] != entry[1]:
    ...
    def remove_bp_breathing(self):
        for current_structure in self.raw_list:
            print('***************')
            print('Index: ' + str(current_structure[0]))
            if current_structure[0] not in self.filter_list and current_structure[0] not in self.bridging_list and current_structure[0] not in self.breathing_list:
                print(current_structure[1])
                for next_structure in self.raw_list[current_structure[0]:]:  # The index stored in line[0] for each line of raw_list starts at 1, so starting from slice [0] here automatically counts up by 1
                    ...
                    if next_structure[0] not in self.filter_list and next_structure[0] not in self.bridging_list and next_structure[0] not in self.breathing_list:
                    ...
                    elif next_structure[0] in self.filter_list:
    ...
    def write_output(self):
        with open(self.dir_name+'/'+self.file_name.replace('complete', 'filtered_30-14_var'), 'w') as pruned_file:
